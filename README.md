...
└──────────────────────────────────────────────────────────────────┘
           │                                    │
    Internet (HTTPS)                    Telegram Platform
           │                                    │
           ▼                                    ▼
    Cloudflare Tunnel                    Telegram Bot API
    (公网入口)                            (消息推送)
           │                                    │
           └────────────┬───────────────────────┘
                        │
...
│                                                                    │
│   ┌─────────────┐                                                │
│   │  Traefik    │  Port 80/443  (反向代理 & TLS 终止)            │
│   │  (v3.0)     │  Port 8080     (Dashboard)                     │
│   └──────┬──────┘                                                │
│          │ HTTP                                                   │
│          ▼                                                         │
│   ┌─────────────────────────────────────────┐                    │
...
```
用户 → Cloudflare Edge → CF Tunnel → Traefik → OpenClaw
```

**🏠 本地访问**（通过自签名证书）：
```
浏览器 → https://openclaw.local/ → Traefik → OpenClaw
```

**💬 Telegram Bot**（推荐主要方式）：
```
用户 → Telegram API → OpenClaw Gateway → AI 引擎 → 回复用户
```
...
| 组件 | 作用 | 端口 | 网络访问 |
|------|------|------|---------|
| **Cloudflare Tunnel** | 公网入口（零配置穿透） | - | 外部 |
| **Traefik** | 反向代理 & 路由 | 80, 443, 8080 | 主机暴露 |
| **OpenClaw Gateway** | AI 助手核心引擎 | 18789 (内部) | 容器内部 |
| **Socat Proxy** | 端口转发代理 | 18790 (外部) | 容器内部 |

📚 **详细架构说明**：[完整架构文档](./docs/ARCHITECTURE.md)

### 关键特性

- ✅ **零配置公网访问**：Cloudflare Tunnel 自动穿透防火墙
- ✅ **多层安全防护**：网络隔离 + TLS 加密 + 设备配对
- ✅ **灵活路由**：Traefik 动态服务发现
- ✅ **数据持久化**：配置、会话、Skills 自动保存
